function validate() {
  ...
  if (email == "abc_123@gmail.com" && Ename == "walkover") {
    alert("OTP has been sent to given Email Id");
    window.location = "otpPage.html";
    return false;
  } else {
    alert("Please enter correct email and username");
  }
}
//need some changes
...
function enableCheck(checkbox) {
  if (checkbox.checked == true) {
    ...
    document.getElementById("startAssessment").style.backgroundColor =
      "#9BC13C";
  } else {
...
function homePage() {
  ...
  while (first) {
    //while a child element is present inside questionsUl
...
function appendAllQuestions() {
  for (var j = 0; j < questions.length; j++) {
    ...
    var divBody =
      "<li style='background-color: grey; border-radius: 30px; padding: 10px 30px; margin-bottom: 10px'>";
    ...
    for (var i = 0; i < numOfOptions; i++) {
      ...
      //active class assigned to this option if it matches the answer
      else divBody += `<li class='optionPanel'>${optionVals[i]}</li>`; //options which does'nt match the answer
    }

    //now lets add edit and remove buttons
    divBody += "<li style='display:flex; justify-content: center;'>";
    divBody +=
      '<button class="btn btn-success fa fa-pencil liBtn" onclick="editRec(this)"></button>';
    divBody +=
      '<button class="btn btn-danger fa fa-trash liBtn" onclick="deleteRec(this)"></button></li>';
    divBody += "</ul></li>";
...
function addQuestion() {
  if (!addingRec) {
    //check if a record is'nt already being added
    addingRec = true;
    //Lets make a UI for adding question //
    var htmlDesign =
      '<li class="panelLi" style="background-color: grey; border-radius: 10px; padding: 10px 11px">';
    htmlDesign += "<h3>Q:&nbsp</h3>";
    htmlDesign += '<input type="text" class="form-control w-75">';
    htmlDesign += '<ul style="width: 700px">';
    // this for loop willbe used to add input field for options
    for (var i = 0; i < 4; i++)
      htmlDesign += `<li style="display: flex;" class="panelLi"><h4>Option ${
        i + 1
      }:&nbsp</h4><input class="form-control w-50"></li>`;
    htmlDesign +=
      '<li style="display: flex;"><h4>Answer:&nbsp</h4><input class="form-control w-50"></li>';
  ...
  if (!(questionVal === "") && !(answerVal === "")) {
    ...
    if (enteredOptions !== 2) alert("Please enter atleast two options");
      ...
      if (!enteredOptions)
        ...
        if (editingRec) {
          //if we're editing a record
          editingRec = false;
          var qNumEditHtml =
            id.parentNode.parentNode.parentNode.firstChild.innerHTML; //this will be used to get the full html of question number we are editing
        ...
        p.innerHTML += divBody;
        document
          .getElementById("questionsUl")
          .insertBefore(
            p.firstChild,
            id.parentNode.parentNode.parentNode.nextSibling
          ); //insert question before the next sibbling of last questions
...
function editRec(id) {
  if (!addingRec) {
    ...
    let answerVal = "";
    let questionValHtml =
      id.parentNode.parentNode.parentNode.childNodes[0].innerHTML; //the full question including "Q: "
    // lets first console.log the value
    let questionVal = questionValHtml.substring(
      questionValHtml.indexOf(";") + 1,
      questionValHtml.length
    );
    ...
    for (var i = 0; i < numOfOtions; i++) {
      ...
      if (
        id.parentNode.parentNode.childNodes[i].className.indexOf("active") !==
        -1
      )
        //if the class of option is 'active', that means that it's the correct answe
    ...
    // this for loop willbe used to add input field for options
    for (var i = 0; i < 4; i++)
      htmlDesign += `<li style="display: flex;" class="panelLi"><h3>Option ${
        i + 1
      }:&nbsp</h3><input class="form-control w-50" value="${
        optionVals[i]
      }"></li>`;
    htmlDesign += `<li style="display: flex;"><h3>Answer:&nbsp</h3><input class="form-control w-50" value="${answerVal}"></li>`;
    htmlDesign += '<li style="display: flex; justify-content: center;">';
    htmlDesign +=
      '<button class="btn btn-success liBtnAdd fa fa-check" onclick="addRec(this)"></button>'; // the same addRec() function will be used
    htmlDesign +=
      '<button class="btn btn-danger liBtnCancel fa fa-times" onclick="cancelUpdateRec(this)"></button>';
    htmlDesign += "</li>";
    htmlDesign += "</ul>";
    htmlDesign += "</li>";
...
function cancelUpdateRec(id) {
  ...
  document
    .getElementById("questionsUl")
    .insertBefore(rVal, id.parentNode.parentNode.parentNode.nextSibling); //add the rVal(as we have stored the previous value here) before the next question
